...
public class Player : MonoBehaviour
{
    ...
    /// <summary>
    /// 총알이 발사될 때 보일 플래시 이펙트 게임 오브젝트
    /// </summary>
    private GameObject flash;

    /// <summary>
    /// 총알이 한 번에 여러발 발사될 때 총알 간의 사이각도
    /// </summary>
    ...
    private int Life
    {
        ...
        {
            // value는 지금 set하는 값
            if (life != value && !isDead) // 값에 변경이 일어났다.
            {                               
                if (life > value )   
                {
    ...
    /// <summary>
    /// 공격력 용 프로퍼티. 1~3 사이의 값을 가진다. 한 번에 발사하는 총알의 숫자와 같다.
    /// </summary>
    private int Power
    {
        get => power;
    ...
    {
        isDead = true;  // 죽었다고 표시
        bodyCollider.enabled = false; // 더 이상 충돌 일어나지 않게 만들기
    ...
    /// <summary>
    /// 이동 부스트 발동용 입력 처리 함수(Shift 눌렀을 때)
    /// </summary>
    /// <param name="context"></param>
    ...
    /// <summary>
    /// 이동 부스트 발동 해제용 입력처리 함수(Shift 뗏을 때)
    /// </summary>
    /// <param name="context"></param>
